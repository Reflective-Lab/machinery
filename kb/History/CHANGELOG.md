...
## 2026-07-08 — Seam A rewire (RFL-171 T4–T5)

Runway re-exported the `helm-event-substrate` contracts and deleted their
in-repo bodies.  All callers in the helms workspace continue to compile with
zero source edits.

### runway-storage (T4)
- Added `helm-event-substrate` as a Cargo dependency (path dep for now,
  `publish = false` until Wave 2 Bedrock import).
- `runway_storage::traits` now re-exports `EventLog`, `SyncableEventLog`,
  `StoredEvent`, `EventQuery`, `LeaseStore`, `LeaseScope`, `LeaseRecord`,
  `AcquireOutcome`, `RenewOutcome`, and `SubstrateError` directly from
  `helm_event_substrate`.  The original trait definitions are deleted; impls
  (`RedbEventLog`, `RedbLeaseStore`, Firestore variants) remain in-crate and
  only the import paths change.
- No source edits required in `helm-session-host`, `helm-coordination`, or
  `helm-governed-jobs` — their existing `use runway_storage::...` imports
  still resolve through the re-exports.

### runway-app-host (T5)
- Deleted `realtime.rs` and `sse.rs` bodies; replaced with re-exports from
  `helm_event_substrate::{EventHub, EventHubHandle, EventEnvelope, EventCursor,
  EventSubscription, sse}`.
- `SessionOwnershipLayer` remains in `runway-app-host` pending RFL-178
  (`OrgIdentity` neutralisation from `runway_auth::AuthContext`).
- `StorageKit`, `RunwayAppHost` builder, and manifest types are unchanged.
