...
| RP-DOC-CANON-FRESH | Curated canon docs carry a freshness stamp (owner + verified-against SHA + last-reviewed) and go stale-gated; generated canon docs (quality/security/performance/gate views) regenerate byte-identically from evidence. A repo's canon references Build-Depot for factory doctrine and never restates it. | Doctrine and ownership split live here (`docs/operations/repository-adoption.md`, Factory-docs / generated-canon rule): repos own product/platform canon plus local-evidence views and reference Build-Depot for gate, scorecard, and RP semantics. Executable enforcement is repo-local and thin-runner-shaped: `bedrock-consolidated` ships `just canon-check` (arena `dim-doc-freshness`) that fails on a stale curated stamp or a non-idempotent generated doc, and emits the verdict as a `documentation_trust` FactorySignal into the Build-Depot graph. Bedrock canon generation (`just canon`) tracked by RFL-224; the gate + this RP's live enforcement tracked by RFL-225. | Aspired (RP + ownership doctrine defined; bedrock generators (RFL-224) and canon-check gate (RFL-225) pending) | — |
