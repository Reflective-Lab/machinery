...
/// SSE (Server-Sent Events) transport — `router`, `encode_frame`, `event_stream`.
/// Implementation lives in `helm-event-substrate`; re-exported here for callers
/// that previously imported from `runway_app_host::sse`.
pub mod sse {
    pub use helm_event_substrate::sse::*;
}
...
pub struct AppExecutionPacket {
    ...
    #[serde(default)]
    pub deploy_contracts: Vec<DeployContract>,
}
...
impl AppExecutionPacket {
    ...
    pub fn with_deploy_contract(
        mut self,
        key: impl Into<String>,
        version: impl Into<String>,
    ) -> Self {
        self.deploy_contracts.push(DeployContract {
            key: key.into(),
...
pub struct JobRegistration {
    /// Validated ambient job key — see [`JobKey`] for why this is not a `String`.
    pub key: JobKey,
    pub display_name: String,
...
impl JobRegistration {
    pub fn new(key: JobKey, display_name: impl Into<String>, source: RegistrationSource) -> Self {
        Self {
            key,
            display_name: display_name.into(),
            source,
...
/// A deploy-time contract the app declares it depends on. The Cloud Run deploy
/// template (D4) materializes the provider-specific env-var slots by matching
/// `key + version` against a Commerce-Rails-published recipe — the app never
/// writes provider env-var names (e.g. `STRIPE_PRICE_*`) into its own repo.
/// D1's manifest verifier validates the field is well-formed and, when
/// `CR_RECIPES_DIR` is set at serve time, that each `key+version` resolves to
/// a Commerce-Rails-published recipe (`<key>@<version>.yaml`).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
...
mod tests {
    ...
    fn app_packet_preserves_registered_jobs_and_modules() {
        let packet = AppExecutionPacket::new("catalyst", "Catalyst", "Ops workflows", "/catalyst")
            .with_job(JobRegistration::new(
                "schedule-strategic-meetings".parse().expect("valid key"),
    ...
    #[test]
    fn packet_parses_deploy_contracts() {
        let json = r#"{
            "app_id": "quorum",
            "display_name": "Q",
            "description": "",
            ...
            "route_prefix": "/q",
            "deploy_contracts": [
                { "key": "commerce-rails-stripe", "version": "0.2" }
            ]
        }"#;
        let pkt = AppExecutionPacket::from_json_str(json).expect("parse");
        assert_eq!(pkt.deploy_contracts.len(), 1);
        assert_eq!(pkt.deploy_contracts[0].key, "commerce-rails-stripe");
        assert_eq!(pkt.deploy_contracts[0].version, "0.2");
    }

    #[test]
    fn packet_defaults_deploy_contracts_to_empty() {
        let json = r#"{
            "app_id": "quorum",
            "display_name": "Q",
            "description": "",
        ...
        let pkt = AppExecutionPacket::from_json_str(json).expect("parse");
        assert!(pkt.deploy_contracts.is_empty());
    }

    #[test]
    fn with_deploy_contract_builder_appends() {
        let pkt = AppExecutionPacket::new("a", "A", "d", "/a")
            .with_deploy_contract("commerce-rails-stripe", "0.2")
            .with_deploy_contract("math-base", "1.96.0");
        assert_eq!(pkt.deploy_contracts.len(), 2);
        assert_eq!(pkt.deploy_contracts[1].key, "math-base");
    }
}
