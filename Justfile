...
# Canonical CI aggregate. GitHub Actions calls exactly this recipe so local and
# CI verdicts stay aligned.
...
deploy-quorum:
    bash ops/scripts/deploy-quorum-server.sh
...
# Build-Depot security gate. Mirrors the blocking cargo-audit workflow.
security-audit:
    #!/usr/bin/env bash
    set -euo pipefail
    ignores=()
    while IFS= read -r id; do
        ignores+=(--ignore "$id")
    done < <(grep -oE 'RUSTSEC-[0-9]+-[0-9]+' audit.toml | sort -u)
    cargo audit --deny warnings "${ignores[@]}"

# Build-Depot delivery gate before production-affecting runtime changes.
...
# ── Deploy templates & math-base (D4) ──────────────────────────────────

# Print the semantic math-base tag derived from docker/Dockerfile.math-base ARGs
math-base-tag:
	@PRINT_TAG=1 ops/scripts/build-math-base.sh

# Build + push kenneth-backend-math-base with its semantic tag (never :latest)
math-base-build:
	ops/scripts/build-math-base.sh

# Drift check: no sibling Dockerfile pins math-base to :latest/@sha256 (RP-NO-MATH-BASE-LATEST)
math-base-audit:
	ops/scripts/math-base-audit.sh

# Show the gcloud env/secret flags an app's deploy_contracts materialize to
deploy-materialize app_dir=".":
	@eval "$(ops/templates/materialize-deploy-contracts.sh {{app_dir}})"; \
	  echo "env-vars: $RUNWAY_DEPLOY_ENV_VARS"; \
	  echo "secrets:  $RUNWAY_DEPLOY_SECRETS"
...
# D5 ship gate: lease tests against the Firestore emulator stack —
# ownership Variant-2 (middleware) + the remote LeaseStore contract suite.
# Mirrors `contract-emulator`'s docker lifecycle (teardown always runs).
# See docs/superpowers/specs/2026-06-15-d5-session-ownership-design.md §9.
test-lease-firestore:
	...
	-FIRESTORE_EMULATOR_HOST=localhost:8080 \
	 PUBSUB_EMULATOR_HOST=localhost:8085 \
	 STORAGE_EMULATOR_HOST=http://localhost:4443 \
	   cargo test -p runway-app-host --test ownership_test variant2_concurrent_contention_against_firestore_emulator -- --nocapture
	-FIRESTORE_EMULATOR_HOST=localhost:8080 \
	 PUBSUB_EMULATOR_HOST=localhost:8085 \
	 STORAGE_EMULATOR_HOST=http://localhost:4443 \
	   cargo test -p runway-storage --test contract_emulator lease_contract -- --ignored --nocapture
	docker compose -f crates/runway-storage/tests/docker-compose.contract.yml down

# RR-STRAT-01: ambient job queue unit tests (no emulators required).
test-ambient:
	cargo test -p runway-ambient
