...
- runtime-runway: CI sibling-checkout machinery removed; workflows authenticate via ssh-agent (deploy-key secret `BEDROCK_SSH_KEY`) so CI proves genuine git-tag resolution; `.cargo/config.toml` (git-fetch-with-cli) now checked in
