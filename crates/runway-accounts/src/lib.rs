...
impl AccountsState {
    pub fn new(storage: Arc<StorageKit>, config: AccountsConfig) -> Self {
        ...
        Self {
            store: AccountStore::new(storage.clone()),
            commerce: CommerceRails::new(
                client.clone(),
    ...
    /// Construct with an externally-supplied `CommerceRails` so callers can
    /// share the same instance (and its internal `Arc<EntitlementStore>`)
    /// between the accounts subsystem and other in-process consumers
    /// (e.g. an entitlement gate on app-specific routes). Avoids drift
    /// between two `CommerceRails::new()` instances, each of which would
    /// otherwise hold its own `EntitlementStore`.
    pub fn with_commerce(
        storage: Arc<StorageKit>,
        config: AccountsConfig,
        commerce: CommerceRails,
    ) -> Self {
        #[allow(clippy::disallowed_methods)]
        let client = reqwest::Client::new();
        Self {
