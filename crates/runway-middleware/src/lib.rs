...
/// Apply only the middleware layers — no `/health` route added.
///
/// Use this when the caller's router already has its own health route (e.g.
/// `runway-app-host`, which mounts `/health` via `health::router()`). Calling
/// `stack()` in that case would add a duplicate `/health` route and panic Axum
/// at startup.
///
/// Order matters: request-id is outermost, compression is innermost.
pub fn apply_layers<S>(router: Router<S>, cfg: &MiddlewareConfig) -> Router<S>
where
...
/// Attach the full middleware stack to any Axum router, adding a `/health`
/// route before applying the layers.
///
/// If your router already has a `/health` route (e.g. `runway-app-host`),
/// use `apply_layers()` instead to avoid a duplicate-route panic.
///
/// Order matters: request-id is outermost, compression is innermost.
...
where
    S: Clone + Send + Sync + 'static,
{
    apply_layers(router.route("/health", get(health)), cfg)
}
